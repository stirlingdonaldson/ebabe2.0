...
export default function VerificationCodeScreen() {
  const { state, setPhoneVerified } = useOnboarding();
  ...
  const handleContinue = useCallback(async () => {
    ...
    try {
      const isVerified = await AuthService.verifyPhoneCode(state.phoneNumber, enteredCode);
      if (isVerified) {
  ...
  }, [code, state.phoneNumber, setPhoneVerified]);
  ...
  const handleResendCode = async () => {
    try {
      ...
      setCountdown(30);
      const success = await AuthService.sendPhoneVerificationCode(state.phoneNumber);
      if (success) {
        setSnackbarMessage(`A new code has been sent to ${state.phoneNumber}`);
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
          <View style={styles.bentoGrid}>
            ...
            <View style={[styles.bentoItem, styles.contentSection]}>
              <Text style={styles.subtitle}>Enter the 6-digit code sent to {state.phoneNumber}</Text>
